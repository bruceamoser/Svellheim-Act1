...
/**
 * build_svellheim_negotiation_tests_pack.js
 *
 * Reads negotiation test JSON files from data/negotiation-tests/ and writes
 * them into a LevelDB compendium pack at module/packs/svellheim-negotiation-tests/.
 *
...
async function main() {
  ...
  console.log(`Found ${jsonFiles.length} negotiation test files in ${path.relative(REPO_ROOT, SOURCE_DIR)}/\n`);
  ...
  console.log(
    `\nWrote pack: ${path.relative(REPO_ROOT, PACK_DIR)} (${items.length} negotiation tests)`
  );
}
